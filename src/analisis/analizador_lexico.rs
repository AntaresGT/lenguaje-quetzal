...
impl AnalizadorLexico {
    ...
    pub fn analizar(&mut self) -> ResultadoQuetzal<Vec<Token>> {
        ...
        while !self.esta_al_final() {
            ...
            if !matches!(
                token.tipo,
                TipoToken::ComentarioLinea(_) | TipoToken::ComentarioBloque(_)
            ) {
                tokens.push(token);
            }
        }
    ...
    fn comentario_linea(&mut self) -> ResultadoQuetzal<Token> {
        ...
        Ok(Token::nuevo(
            TipoToken::ComentarioLinea(comentario.clone()),
            format!("//{}", comentario),
            linea,
            columna,
        ))
    ...
    fn comentario_bloque(&mut self) -> ResultadoQuetzal<Token> {
        ...
        Ok(Token::nuevo(
            TipoToken::ComentarioBloque(comentario.clone()),
            format!("/*{}*/", comentario),
            linea,
            columna,
        ))
                        ...
                        match escape {
                            ...
                            _ => {
                                return Err(ErrorQuetzal::ErrorSintaxis {
                                    linea: self.linea_actual,
                                    mensaje: format!(
                                        "Secuencia de escape no válida en interpolación: \\{}",
                                        escape
                                    ),
                                });
                            }
            ...
            if self.mirar() == Some('{') {
                ...
                while nivel_llaves > 0 && !self.esta_al_final() {
                    if let Some(c) = self.mirar() {
                        ...
                        return Err(ErrorQuetzal::ErrorSintaxis {
                            linea: self.linea_actual,
                            mensaje: "Expresión de interpolación sin cerrar (falta '}')"
                                .to_string(),
                        });
                    }
                }
