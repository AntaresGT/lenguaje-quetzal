...
use super::analizador_lexico::{AnalizadorLexico, SegmentoTexto, TipoToken, Token};
...
impl AnalizadorSintactico {
            ...
            if self.posicion_actual + 1 < self.tokens.len() {
                ...
                if let TipoToken::Identificador(_) = &self.tokens[self.posicion_actual + 1].tipo {
                    if self.posicion_actual + 2 < self.tokens.len() {
                        if matches!(
                            &self.tokens[self.posicion_actual + 2].tipo,
                ...
                if matches!(&self.tokens[self.posicion_actual + 1].tipo, TipoToken::Var) {
                    if self.posicion_actual + 2 < self.tokens.len() {
                        if let TipoToken::Identificador(_) =
                            &self.tokens[self.posicion_actual + 2].tipo
                        {
                            if self.posicion_actual + 3 < self.tokens.len() {
                                if matches!(
                                    &self.tokens[self.posicion_actual + 3].tipo,
                                    TipoToken::Asignacion
                                ) {
                                    return self.declaracion_variable();
                                }
            ...
            TipoToken::Y | TipoToken::O => {
                ...
                if self.posicion_actual + 1 < self.tokens.len() {
                    match &self.tokens[self.posicion_actual + 1].tipo {
                        TipoToken::Asignacion
                        | TipoToken::AsignacionSuma
                        | TipoToken::AsignacionResta
                        | TipoToken::AsignacionMult
                        | TipoToken::AsignacionDiv
                        | TipoToken::AsignacionMod => {
                            let nombre = if matches!(self.token_actual().tipo, TipoToken::Y) {
                                "y"
                            } else {
                                "o"
                            };
                            return Err(ErrorQuetzal::ErrorSintaxis {
                                ...
                                mensaje: format!(
                                    "no se puede asignar a la palabra reservada `{}`",
                                    nombre
                                ),
                            });
                        }
        ...
        if self.es_tipo_dato(&self.tokens[pos].tipo) {
            ...
            if matches!(self.tokens[pos - 1].tipo, TipoToken::TipoLista) {
                if pos < self.tokens.len() && matches!(self.tokens[pos].tipo, TipoToken::Menor) {
                    ...
                    // Debe haber >
                    if pos >= self.tokens.len()
                        || !matches!(self.tokens[pos].tipo, TipoToken::Mayor)
                    {
                        return false;
                    }
                    pos += 1;
            ...
            loop {
                ...
                let alias = if self.coincidir(&TipoToken::Como) {
                    if let TipoToken::Identificador(nom_alias) = &self.token_actual().tipo {
                        ...
                        return Err(ErrorQuetzal::ErrorSintaxis {
                            linea: self.token_actual().linea,
                            mensaje: "Se esperaba el nombre del alias después de 'como'"
                                .to_string(),
                        });
                    }
                } else {
    ...
    fn declaracion_exportar(&mut self) -> ResultadoQuetzal<Nodo> {
        ...
        Ok(Nodo::DeclaracionExportar { elementos, linea })
    }
            ...
            if self.coincidir(&TipoToken::Libre) {
                ...
                if es_funcion {
                    ...
                    } = funcion
                    {
                        ...
                        miembros.push(Nodo::DeclaracionFuncion {
                            nombre,
                            parametros,
                            tipo_retorno,
                            cuerpo,
                    ...
                    } = variable
                    {
                        ...
                        miembros.push(Nodo::DeclaracionVariable {
                            nombre,
                            tipo_dato,
                            es_variable,
                            valor,
    ...
    fn declaracion_constructor_palabra_clave(
        &mut self,
        _nombre_clase: &str,
    ) -> ResultadoQuetzal<Nodo> {
        let linea = self.token_actual().linea;
        ...
        if !self.verificar(&TipoToken::ParentesisCierra) {
            loop {
                ...
                } else if let TipoToken::Identificador(tipo_personalizado) =
    ...
    fn declaracion_variable(&mut self) -> ResultadoQuetzal<Nodo> {
        ...
        let nombre = match &self.token_actual().tipo {
            ...
            TipoToken::Y => {
                ...
                return Err(ErrorQuetzal::ErrorSintaxis {
                    linea: self.token_actual().linea,
                    mensaje: "no se puede usar la palabra reservada `y` como nombre de variable"
                        .to_string(),
                });
            }
            TipoToken::O => {
                ...
                return Err(ErrorQuetzal::ErrorSintaxis {
                    linea: self.token_actual().linea,
                    mensaje: "no se puede usar la palabra reservada `o` como nombre de variable"
                        .to_string(),
                });
            }
            _ => {
    ...
    fn comparacion(&mut self) -> ResultadoQuetzal<Nodo> {
        ...
        while self.coincidir(&TipoToken::Mayor)
            || self.coincidir(&TipoToken::MayorIgual)
            || self.coincidir(&TipoToken::Menor)
            || self.coincidir(&TipoToken::MenorIgual)
        {
            let operador = self.token_anterior().lexema.clone();
    ...
    fn acceso_miembro(&mut self) -> ResultadoQuetzal<Nodo> {
        ...
        loop {
            if self.coincidir(&TipoToken::Punto) {
                ...
                let miembro =
                    if let TipoToken::Identificador(nombre_miembro) = &self.token_actual().tipo {
    ...
    fn primario(&mut self) -> ResultadoQuetzal<Nodo> {
        ...
        if self.coincidir(&TipoToken::Nuevo) {
            ...
            if !self.coincidir(&TipoToken::ParentesisCierra) {
                return Err(ErrorQuetzal::ErrorSintaxis {
                    linea: self.token_actual().linea,
                    mensaje: "Se esperaba ')' después de los argumentos del constructor"
                        .to_string(),
                });
            }
                ...
                if self.coincidir(&TipoToken::LlaveAbre) {
                    ...
                    if !self.verificar(&TipoToken::LlaveCierra) {
                        loop {
                            ...
                            let clave = if let TipoToken::Identificador(nom) =
                                &self.token_actual().tipo
                            {
                                ...
                                let tipo_como_clave = match &self.token_actual().tipo {
                                    ...
                                    _ => {
                                        return Err(ErrorQuetzal::ErrorSintaxis {
                                            linea: self.token_actual().linea,
                                            mensaje:
                                                "Tipo de dato no válido como clave de propiedad"
                                                    .to_string(),
                                        })
                                    }
                                };
                                ...
                                &self.token_actual().tipo,
                                TipoToken::Verdadero | TipoToken::Falso | TipoToken::Nulo
                            ) {
                                ...
                                let palabra_como_clave = match &self.token_actual().tipo {
                                    ...
                                    _ => return Err(ErrorQuetzal::ErrorSintaxis {
                                        linea: self.token_actual().linea,
                                        mensaje:
                                            "Palabra reservada no válida como clave de propiedad"
                                                .to_string(),
                                    }),
                                };
                                self.avanzar();
                            ...
                            if !self.coincidir(&TipoToken::DosPuntos) {
                                return Err(ErrorQuetzal::ErrorSintaxis {
                                    linea: self.token_actual().linea,
                                    mensaje: "Se esperaba ':' después del nombre de la propiedad"
                                        .to_string(),
                                });
                            }
    ...
    fn verificar_siguiente(&self, tipo: &TipoToken) -> bool {
        ...
        std::mem::discriminant(&self.tokens[self.posicion_actual + 1].tipo)
            == std::mem::discriminant(tipo)
    }
        ...
        if self.coincidir(&TipoToken::ParentesisAbre) {
            ...
            if es_foreach {
                ...
                let variable = if let TipoToken::Identificador(nom) = &self.token_actual().tipo {
                    ...
                    return Err(ErrorQuetzal::ErrorSintaxis {
                        linea: self.token_actual().linea,
                        mensaje: "Se esperaba el nombre de la variable en bucle foreach"
                            .to_string(),
                    });
                };
                ...
                if !self.coincidir(&TipoToken::PuntoYComa) {
                    return Err(ErrorQuetzal::ErrorSintaxis {
                        linea: self.token_actual().linea,
                        mensaje: "Se esperaba ';' después de la inicialización del bucle para"
                            .to_string(),
                    });
                }
                ...
                if !self.coincidir(&TipoToken::PuntoYComa) {
                    return Err(ErrorQuetzal::ErrorSintaxis {
                        linea: self.token_actual().linea,
                        mensaje: "Se esperaba ';' después de la condición del bucle para"
                            .to_string(),
                    });
                }
                ...
                let incremento = if self.coincidir(&TipoToken::ParentesisCierra) {
                    ...
                    if !self.coincidir(&TipoToken::ParentesisCierra) {
                        return Err(ErrorQuetzal::ErrorSintaxis {
                            linea: self.token_actual().linea,
                            mensaje: "Se esperaba ')' después del incremento del bucle para"
                                .to_string(),
                        });
                    }
                    inc
    ...
    fn bucle_hacer_mientras(&mut self) -> ResultadoQuetzal<Nodo> {
        ...
        if !self.coincidir(&TipoToken::Mientras) {
            return Err(ErrorQuetzal::ErrorSintaxis {
                linea: self.token_actual().linea,
                mensaje: "Se esperaba 'mientras' después del bloque en bucle hacer-mientras"
                    .to_string(),
            });
        }
    ...
    fn bloque_intentar(&mut self) -> ResultadoQuetzal<Nodo> {
        ...
        while self.coincidir(&TipoToken::Capturar) {
            ...
            // Nombre de la variable
            let nombre_variable =
                if let TipoToken::Identificador(nombre) = &self.token_actual().tipo {
                    let n = nombre.clone();
                    self.avanzar();
                    n
                } else {
        ...
        if bloques_capturar.is_empty() {
            return Err(ErrorQuetzal::ErrorSintaxis {
                linea,
                mensaje: "Se esperaba al menos un bloque 'capturar' después de 'intentar'"
                    .to_string(),
            });
        }
        ...
        for segmento in segmentos {
            match segmento {
                ...
                SegmentoTexto::Expresion(expresion_str) => {
                    ...
                    let mut analizador_expresion = AnalizadorLexico::nuevo(&expresion_str);
                    let tokens_expresion = analizador_expresion.analizar().map_err(|_| {
                        ErrorQuetzal::ErrorSintaxis {
                            linea,
                            mensaje: format!(
                                "Error en la expresión de interpolación: '{}'",
                                expresion_str
                            ),
                        }
                    })?;
                    ...
                    let mut analizador_sintactico_expresion =
                        AnalizadorSintactico::nuevo(tokens_expresion);
                    let nodo_expresion =
                        analizador_sintactico_expresion.expresion().map_err(|_| {
                            ErrorQuetzal::ErrorSintaxis {
                                linea,
                                mensaje: format!(
                                    "Error al analizar la expresión de interpolación: '{}'",
                                    expresion_str
                                ),
                            }
                        })?;
                    ...
                    segmentos_procesados
                        .push(SegmentoInterpolacion::Expresion(Box::new(nodo_expresion)));
                }
            }
        }
