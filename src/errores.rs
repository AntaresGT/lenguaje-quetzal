...
impl ErrorQuetzal {
    ...
    pub fn mostrar_error_con_codigo(&self, codigo_fuente: Option<&str>, nombre_archivo: Option<&str>) {
        match self {
            ErrorQuetzal::ErrorSintaxis { linea, mensaje } => {
                ...
                if mensaje.contains("palabra reservada") {
                    self.mostrar_error_formato_rust("error", "E0106", *linea, mensaje, codigo_fuente, nombre_archivo, 
                        Some("las palabras reservadas están predefinidas por el lenguaje"), 
                        Some("usa un nombre diferente para tu variable"));
                } else {
                    self.mostrar_error_formato_rust("error", "E0001", *linea, mensaje, codigo_fuente, nombre_archivo, None, Some("verifica la sintaxis del código"));
                }
            },
            ErrorQuetzal::ErrorTipo { linea, mensaje } => {
                self.mostrar_error_formato_rust("error", "E0002", *linea, mensaje, codigo_fuente, nombre_archivo, None, Some("verifica que los tipos sean compatibles"));
            },
            ErrorQuetzal::ErrorEjecucion { linea, mensaje } => {
                self.mostrar_error_formato_rust("error", "E0003", *linea, mensaje, codigo_fuente, nombre_archivo, None, None);
            },
            ErrorQuetzal::VariableNoDefinida { linea, nombre } => {
                ...
                self.mostrar_error_formato_rust("error", "E0425", *linea, &mensaje, codigo_fuente, nombre_archivo, None, sugerencia);
            },
            ErrorQuetzal::FuncionNoDefinida { linea, nombre } => {
                let mensaje = format!("no se puede encontrar la función `{}` en este ámbito", nombre);
                let sugerencia = Some("verifica que la función esté definida antes de usarla");
                self.mostrar_error_formato_rust("error", "E0425", *linea, &mensaje, codigo_fuente, nombre_archivo, None, sugerencia);
            },
            ErrorQuetzal::VariableInmutable { linea, nombre } => {
                let mensaje = format!("no se puede asignar dos veces a la variable inmutable `{}`", nombre);
                let sugerencia = Some("considera hacer la variable mutable: `var`");
                self.mostrar_error_formato_rust("error", "E0384", *linea, &mensaje, codigo_fuente, nombre_archivo, None, sugerencia);
            },
            ErrorQuetzal::DivisionPorCero { linea } => {
                ...
                self.mostrar_error_formato_rust("error", "E0080", *linea, mensaje, codigo_fuente, nombre_archivo, None, sugerencia);
            },
            ErrorQuetzal::IndiceFueraDeRango { linea, indice, tamanio } => {
                let mensaje = format!("el índice {} está fuera de los límites de una lista de longitud {}", indice, tamanio);
            ...
            ErrorQuetzal::ConversionInvalida { linea, tipo_origen, tipo_destino } => {
                let mensaje = format!("no se puede convertir el tipo `{}` al tipo `{}`", tipo_origen, tipo_destino);
                let sugerencia = Some("verifica que la conversión sea válida");
                self.mostrar_error_formato_rust("error", "E0308", *linea, &mensaje, codigo_fuente, nombre_archivo, None, sugerencia);
            },
            ErrorQuetzal::ArgumentosIncorrectos { linea, esperados, recibidos } => {
                let mensaje = format!("esta función toma {} argumentos pero se proporcionaron {}", esperados, recibidos);
                let sugerencia = Some("verifica el número de argumentos en la llamada");
                self.mostrar_error_formato_rust("error", "E0061", *linea, &mensaje, codigo_fuente, nombre_archivo, None, sugerencia);
            },
            ErrorQuetzal::TokenInesperado { linea, token } => {
                ...
                self.mostrar_error_formato_rust("error", "E0001", *linea, &mensaje, codigo_fuente, nombre_archivo, None, sugerencia);
            },
            ErrorQuetzal::FinArchivoInesperado => {
                eprintln!("{}[E0001]: fin de archivo inesperado", "error".red().bold());
                eprintln!("  {} el archivo terminó de forma inesperada", "=".blue().bold());
            },
            ErrorQuetzal::ErrorImportacion { mensaje } => {
                eprintln!("{}[E0432]: {}", "error".red().bold(), mensaje);
            },
            ErrorQuetzal::ModuloNoEncontrado { linea, ruta, detalle } => {
                ...
                self.mostrar_error_formato_rust("error", "E0432", *linea, &mensaje, codigo_fuente, nombre_archivo, nota, sugerencia);
            },
            ErrorQuetzal::ErrorCargaModulo { linea, ruta, detalle } => {
            ...
            ErrorQuetzal::ElementoNoExportado { linea, elemento, modulo, sugerencia } => {
                ...
                self.mostrar_error_formato_rust("error", "E0432", *linea, &mensaje, codigo_fuente, nombre_archivo, nota, ayuda);
            },
            ErrorQuetzal::ErrorExportacion { linea, elemento, sugerencia } => {
                let mensaje = format!("no se puede exportar `{}` porque no está definido", elemento);
                let nota = Some(sugerencia.as_str());
                let ayuda = Some("define la variable antes de exportarla");
                self.mostrar_error_formato_rust("error", "E0425", *linea, &mensaje, codigo_fuente, nombre_archivo, nota, ayuda);
            },
            ErrorQuetzal::RutaModuloInvalida { linea, ruta, razon } => {
                ...
                let sugerencia = Some("usa rutas relativas desde el archivo principal o rutas absolutas válidas");
                self.mostrar_error_formato_rust("error", "E0432", *linea, &mensaje, codigo_fuente, nombre_archivo, nota, sugerencia);
            },
            ErrorQuetzal::DependenciaCircular { linea, modulo, cadena } => {
                let mensaje = format!("dependencia circular detectada en el módulo `{}`", modulo);
                let nota = Some(cadena.as_str());
                let sugerencia = Some("reorganiza los módulos para evitar importaciones circulares");
                self.mostrar_error_formato_rust("error", "E0369", *linea, &mensaje, codigo_fuente, nombre_archivo, nota, sugerencia);
            },
            ErrorQuetzal::ErrorInterno { mensaje } => {
                eprintln!("{}[E0999]: error interno del intérprete", "error".red().bold());
                eprintln!("  {} {}", "=".blue().bold(), mensaje);
                eprintln!("  {} esto es un bug en el intérprete, por favor reporta este error", "=".blue().bold());
            },
            ErrorQuetzal::ErrorConversion { linea, mensaje } => {
                let sugerencia = Some("verifica que el valor se pueda convertir al tipo solicitado");
                self.mostrar_error_formato_rust("error", "E0308", *linea, mensaje, codigo_fuente, nombre_archivo, None, sugerencia);
            },
        }
    }
    ...
    fn mostrar_codigo_con_indicador(&self, codigo: &str, linea_error: usize) {
        ...
        for (i, linea_contenido) in lineas.iter().enumerate().skip(inicio).take(fin - inicio) {
            ...
            if numero_linea == linea_error {
                ...
                eprintln!("  {} {}", 
                    "|".blue().bold(),
                    format!("{}^ aquí está el error", " ".repeat(self.calcular_posicion_error(linea_contenido))).red().bold()
                );
            } else {
                // Líneas de contexto
                eprintln!("{} {} {}", 
