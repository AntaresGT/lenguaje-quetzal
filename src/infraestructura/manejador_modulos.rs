...
use crate::analisis::analizador_lexico::{AnalizadorLexico, TipoToken};
...
impl ManejadorModulos {
    ...
    pub fn nuevo(
        ...
        let ruta_principal = Path::new(ruta_principal).canonicalize().map_err(|error| {
            ErrorQuetzal::ErrorInterno {
                mensaje: format!(
                    "No se pudo resolver la ruta principal '{}': {}",
                    ruta_principal, error
                ),
            }
        })?;
    ...
    pub fn procesar_importacion(
        ...
        let modulo = self.modulos_cargados.get(&clave_modulo).ok_or_else(|| {
            ...
            if let Some(elemento_exportado) = modulo.exportaciones.get(nombre_real) {
                ...
                match elemento_exportado {
                    ElementoExportado::Variable(variable) => {
                        evaluador
                            .definir_variable_global(nombre_local.clone(), variable.clone())?;
                    }
                    ...
                    ElementoExportado::Instancia(instancia) => {
                        ...
                        evaluador
                            .definir_variable_global(nombre_local.clone(), instancia.clone())?;
                    }
                }
            } else {
                // Crear sugerencia de elementos disponibles
                let elementos_disponibles: Vec<String> =
                    modulo.exportaciones.keys().cloned().collect();
                let sugerencia = if elementos_disponibles.is_empty() {
                    ...
                    format!(
                        ...
                        elementos_disponibles.join(", ")
                    )
                } else {
                    format!(
                        "Elementos disponibles: {} y {} más",
    ...
    pub fn procesar_exportacion(
        ...
        for nombre in elementos {
            ...
            if let Some(variable) = evaluador.obtener_variable(nombre) {
                ...
                match &variable.valor {
                    crate::datos::tipos_datos::Valor::Objeto { .. } => {
                        exportaciones
                            .insert(nombre.clone(), ElementoExportado::Instancia(variable));
                    }
                    _ => {
        ...
        let ruta_absoluta = Path::new(ruta_actual).canonicalize().map_err(|error| {
            ErrorQuetzal::ErrorCargaModulo {
                linea,
        ...
        if let Some(modulo) = self
            .modulos_cargados
            .get_mut(&ruta_absoluta.to_string_lossy().to_string())
        {
    ...
    fn resolver_ruta_modulo(&self, ruta_modulo: &str, linea: usize) -> ResultadoQuetzal<PathBuf> {
        ...
        if ruta_modulo.contains("..") {
            return Err(ErrorQuetzal::RutaModuloInvalida {
                linea,
                ruta: ruta_modulo.to_string(),
                razon: "No se permite el uso de '..' en las rutas de módulos por seguridad"
                    .to_string(),
            });
        }
        ...
        if ruta_candidata.exists() {
            ...
            let detalle = if archivos_cercanos.is_empty() {
                format!(
                    "No se encontraron archivos .qz en el directorio '{}'",
                    directorio_principal.display()
                )
            } else if archivos_cercanos.len() <= 5 {
                format!(
                    "Archivos .qz disponibles en el directorio: {}",
                    archivos_cercanos.join(", ")
                )
            } else {
                format!(
                    "Archivos .qz disponibles: {} y {} más",
    ...
    fn cargar_modulo(
        ...
        let codigo = fs::read_to_string(ruta).map_err(|error| {
            let detalle = match error.kind() {
                ...
                std::io::ErrorKind::PermissionDenied => {
                    "Sin permisos para leer el archivo".to_string()
                }
                std::io::ErrorKind::InvalidData => {
                    "El archivo contiene datos inválidos o no es UTF-8".to_string()
                }
        ...
        let tokens_filtrados: Vec<_> = tokens
            .into_iter()
            .filter(|token| !matches!(token.tipo, TipoToken::NuevaLinea))
            .collect();
        ...
        let entorno_anterior =
            evaluador.intercambiar_entorno(Rc::new(RefCell::new(entorno_modulo)));

        // Establecer la ruta del archivo actual para las exportaciones
        let ruta_anterior = evaluador
            .obtener_ruta_archivo_actual()
            .map(|s| s.to_string());
        ...
        let exportaciones_encontradas = match resultado_evaluacion {
            Ok(_) => {
                ...
                let mut exportaciones = HashMap::new();
                match self.recopilar_exportaciones_del_ast(
                    &ast,
                    evaluador,
                    ruta,
                    &mut exportaciones,
    ...
    fn recopilar_exportaciones_en_nodo(
        ...
        match nodo {
            Nodo::Programa(declaraciones) => {
                for declaracion in declaraciones {
                    self.recopilar_exportaciones_en_nodo(
                        declaracion,
                        evaluador,
                        ruta,
                        exportaciones,
                        ...
                        match &variable.valor {
                            crate::datos::tipos_datos::Valor::Objeto { .. } => {
                                exportaciones.insert(
                                    elemento.clone(),
                                    ElementoExportado::Instancia(variable),
                                );
                            }
                            _ => {
                                exportaciones.insert(
