...
use crate::analisis::analizador_sintactico::AnalizadorSintactico;
...
pub fn interpretar_archivo_con_modulos(ruta: &str) -> ResultadoQuetzal<Valor> {
    ...
    let codigo = fs::read_to_string(ruta).map_err(|error| ErrorQuetzal::ErrorInterno {
...
pub fn modo_interactivo() {
    ...
    println!(
        "{}",
        "=== Intérprete Interactivo de Quetzal ===".cyan().bold()
    );
    println!(
        "{}",
        "Escribe código Quetzal y presiona Enter para ejecutarlo.".yellow()
    );
...
#[allow(dead_code)]
fn interpretar_linea_interactiva(
    codigo: &str,
